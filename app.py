    ...
    try:
        ...
        system_prompt = {
            ...
            'parts': [
                ...
                "```schedule\n"
                "語法說明：活動格式: [星期] [開始時間]-[結束時間] [活動名稱] [[備註]]星期: 週一, 週二... 或 一, 二...日;時間: 24小時制，例如 09:00, 14:30;跨天: 週一 23:00-次日 01:30 夜間學習;備註: 在行末使用方括號 [這是我的備註];全局設定: config:ics_repeat=3m (設定日曆匯出重複3個月，預設6個月)\n"
                "```\n"
                "例如：\n"
                "```schedule\n"
                ...
                "要求 1.你必須帶點主觀，給予用戶建議，如一個小時的課程，需要再額外抽一個小時的複習時間。或是，要不要安排健身活動等。 2.你的主觀意見在沒有用戶的同意下，不該自行加入行事曆中。 3.你也可以詢問用戶這樣的作息表要持續多久 4.語法上，時間只能在整點或三十分，所以根據用戶的需求，你必須要強制進位到最近的點上。"
            ]
        }
